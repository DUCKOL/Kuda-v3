...
const geminiApiKeys = apiKeysFromEnv.map((key) => key.trim()).filter((key) => key.length > 0).map((key, index) => ({
  key,
  index: index + 1
}));
if (geminiApiKeys.length === 0) {
  console.warn(
    '[API Service] 警告: 未在环境变量中配置 GEMINI_API_KEYS。AI 相关功能将无法使用。请在项目根目录的 .env 文件中进行配置，例如: GEMINI_API_KEYS="key1,key2,key3"'
  );
}
async function realGeminiApiCall(keyInfo, prompt, imageBase64) {
...
async function callGeminiWithFallback(prompt, imageBase64) {
  if (geminiApiKeys.length === 0) {
...
function createMainWindow() {
  mainWindow = new electron.BrowserWindow({
    ...
    webPreferences: {
      ...
      // 注意：为了简单起见保持false，但官方推荐true + contextBridge
    },
    alwaysOnTop: true,
    frame: false,
    skipTaskbar: true,
...
electron.app.whenReady().then(() => {
  ...
  fullscreenCheckInterval = setInterval(checkFullscreen, 1e3);
  electron.ipcMain.handle("ask-ai", async (event, { promptText, imageBase64 }) => {
  ...
  electron.ipcMain.on("request-screenshot", captureScreenshotAndSendToRenderer);
  const screenshotShortcut = "Shift+Z";
  if (!electron.globalShortcut.register(screenshotShortcut, captureScreenshotAndSendToRenderer)) {
    console.log(`快捷键 ${screenshotShortcut} 注册失败`);
  }
  const toggleModelShortcut = "Alt+Z";
  if (!electron.globalShortcut.register(toggleModelShortcut, () => {
